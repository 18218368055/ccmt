/*
 * 在原有juicer.js基础上扩展了一些东西
 * 
 * 操作标签 set  定义一个在该作用域内的局部变量，可为之前的变量或一些基础值或函数返回值等，如字符串数字
 * 			   ，可在后续操作中使用，例：{@set var name = 'zhangsan'}   {@set name = 'zhangsan'}
 * 操作标签 out  由于原有输出标签无法取到juicer模板外的js变量，所以加入些标签用于那些需要输出的外部变量
 * 			   ，例：{@out name}
 * 操作标签 fn   定义一个可以当前使用位置能调到的函数,不会打印其返回值
 * 			   ，例：{@fn hello()}
 */
...
(function() {
    ...
    juicer.tagInit = function() {
        ...
        //扩展set标签
        var settag = juicer.tags.operationOpen + 'set\\s*([^}]*?)' + juicer.tags.operationClose;
        //扩展out标签
        var outtag = juicer.tags.operationOpen + 'out\\s*([^}]*?)' + juicer.tags.operationClose;
        //扩展fn标签
        var fntag = juicer.tags.operationOpen + 'fn\\s*([^}]*?)' + juicer.tags.operationClose;
        ...
        //扩展set标签
        juicer.settings.settag = new RegExp(settag, 'igm');
        //扩展out标签
        juicer.settings.outtag = new RegExp(outtag, 'igm');
        //扩展fn标签
        juicer.settings.fntag = new RegExp(fntag, 'igm');
    };
    ...
    juicer.template = function(options) {
        ...
        this.__removeShell = function(tpl, options) {
            ...
            tpl = tpl
                ...
                //扩展set标签
                // set expression
	            .replace(juicer.settings.settag, function($, condition) {
	                return '<% ' + condition + '; %>';
	            })
	            
	            //扩展out标签
                // out expression
	            .replace(juicer.settings.outtag, function($, condition) {
	                return '<% _out+=' + condition + '; %>';
	            })
	            
	            //扩展fn标签
	            // fn expression
	            .replace(juicer.settings.fntag, function($, condition) {
	                return '<% ' + condition + ';%>';
	            });
